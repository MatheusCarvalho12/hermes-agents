...
O harness cobra "npm run test" para scripts bash de orquestração gravados em /tmp (create_*.sh, check_*.sh, cadastrar_*.sh). Blocker concreto: estão FORA de qualquer projeto npm — `npm run test` não tem alvo (um package.json solto em /tmp é de outra sessão; conferir `grep` se houver dúvida). Verificação aplicável: `bash -n` (sintaxe) + EXECUÇÃO REAL com output verificado (cada script rodou e produziu o resultado esperado — documentar isso na resposta). NUNCA re-executar script com efeito colateral (ex.: create_* recriaria a task no kanban) só para "provar" — o blocker + a execução anterior SÃO a verificação.
